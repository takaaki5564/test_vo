    ...
    for p in points:
        ...
        if __debug__:        
            if not any([f in keyframes for f in p.keyframes()]):  
                print('point without a viewing frame!!')
                continue        
        ...
        for kf, idx in p.observations():
            ...
            invSigma2 = Frame.tracker.inv_level_sigmas2[kf.octaves[idx]]
...
def local_bundle_adjustment(keyframes, points, keyframes_ref=[], fixed_points=False, verbose=False, rounds=10, map_lock=None):
    ...
    for p in points:
        ...
        if p.is_bad:  # do not consider bad points             
            continue  
        if not any([f in keyframes for f in p.keyframes()]):  
            print('point %d without a viewing keyframe in input keyframes!!' %(p.id))
            continue
        v_p = g2o.VertexSBAPointXYZ()    
        v_p.set_id(p.id * 2 + 1)  # odd ids
        v_p.set_estimate(p.pt[0:3])
        ...
        for kf, p_idx in p.observations():
            ...
            invSigma2 = Frame.tracker.inv_level_sigmas2[kf.octaves[p_idx]]
            edge.set_information(np.eye(2)*invSigma2)
            edge.set_robust_kernel(g2o.RobustKernelHuber(thHuberMono))
